...
class SnakeGame:
    def __init__(self, root, Agent, mode, epochs, visual=None, sbs="off"):
        self.visual = visual
        self.step_by_step = sbs
        self.mode = mode
        self.epochs = epochs
        ...
        if mode == "demo" or self.visual == "on":
            self.canvas = tk.Canvas(root, width=W, height=H, bg="lightgrey")
            self.canvas.pack()
    ...
    def move_snake(self):
        ...
        if new_head == self.food_red:
            ...
            empty = [(x, y) for x in range(GRID_SIZE) for y in range(GRID_SIZE)
                     if (x, y) not in self.snake
                     and (x, y) not in self.food_green]
        ...
        # Eat green food
            ...
            empty = [(x, y) for x in range(GRID_SIZE) for y in range(GRID_SIZE)
                     if (x, y) not in self.snake and (x, y) != self.food_red
                     and (x, y) not in self.food_green]
